...
public interface ComBusinessRepository extends JpaRepository<ComBusiness,Integer> {
    ...
    @Query(value = "select * from com_business where business_desc=?1",nativeQuery = true)
    ComBusiness findByBusinessDesc(String businessDesc);

    @Query(value = "select * from com_business where business_desc like %?1%",nativeQuery = true)
    ...
    @Query(value = "select * from com_business where business_type_id=?1",nativeQuery = true)
